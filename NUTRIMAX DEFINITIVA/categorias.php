...
try {
        // 1. Intentar cargar las categorías.
        // Si hay un fallo de BD, se lanza una Exception aquí.
        $categorias = cargar_categorias();
        
        // 2. Si es exitoso pero no hay resultados:
        if (empty($categorias)) {
            $error = "No hay categorías disponibles.";
        }
        
    } catch (Exception $e) {
        // 3. Capturar cualquier fallo de la base de datos.
        $error = "Error al cargar las categorías: " . htmlspecialchars($e->getMessage());
        $categorias = []; // Aseguramos que el array esté vacío para no intentar iterar.
    }
...
<html>
	...
	<body class="body">
	   ...
	   <?php
		// guardamos el emial si existe y no es nulo para el localstorage
		$email_a_inyectar = isset($_SESSION['usuario']['gmail']) ? $_SESSION['usuario']['gmail'] : null;
		?>
<script>
 
    const userEmail = <?php echo json_encode($email_a_inyectar); ?>;

    // ¡AGREGAR ESTA VERIFICACIÓN!
    if (userEmail !== null && typeof userEmail === 'string') { 
        // Si el email es válido (no nulo Y es una cadena), entonces:
        if (leerEmailDeLocalStorage() === null) {
            guardarEmailEnLocalStorage(userEmail); 
            console.log("Email guardado en Local Storage por primera vez: " + userEmail);
        }
    }
    // ...
</script>
