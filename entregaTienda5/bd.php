...
function cargar_productos_carrito_pendiente($codCarroExistente){
    ...
    // 1. Sentencia SQL con marcador de posición (?)
    // Asumo una tabla 'productos_carro' que une el carrito con los productos.
    $sql = "SELECT CodProd, Unidades FROM carroproductos WHERE CodCarro = ?";
    
    // 2. Preparar el statement
	$stmt = $bd->prepare($sql);
        
        // 3. Vincular el parámetro (codCarro)
...
function insertar_pedido($carrito, $codRes, $idcarro){
    ...
    // 1. Actualizamos el total del carro existente
    $total_calculado = calcular_total_carrito($carrito);
    ...
    foreach($carrito as $codProd => $unidades){
        ...
        // A) Comprobar si este producto YA existe en este carro en la BD
...
function completar_pedido($carrito, $codRes, $idcarro){
    ...
    // =============================================================
    // MODIFICACIÓN CRUCIAL: ELIMINACIÓN COMPLETA DEL CARRO VACÍO
    // =============================================================
    if (empty($carrito)) {
