...
$cadena_conexion = 'mysql:dbname=simulador_tienda;host=127.0.0.1;port=3306';
$usuario = 'root';
$clave = '';
...
//funcion para calcular el total de un carro de productos, cuidado al usar array keys hay que pasarlo el carro directamente 
...
//funcion para insertar pedido nuevo osea este usuario no tenia un carro pendiente cuando añades un producto en detalledetalle
...
//funcion que te elimina un producto de una linea de producto de un carro especifico PENDIENTE
//para que cuando borremos en carro pero no completemos el pedido nos borre de la bd esa linea de producto

function eliminar_producto_carro_pendiente($codProd){
$bd = leer_config();
$gmail_del_usuario = $_SESSION['usuario']['gmail'];//obtenemos el gmail del usuario


    //como esta funcion devuelve varios datos, primero la llamamos
    $datosCarro = cargar_carro_pendiente($gmail_del_usuario); 
    
    //Comprueba que se encontró el carrito.
    if (!$datosCarro) {
        return FALSE; // No hay carrito pendiente.
    }
    
    //PASO CLAVE: Obtener el ID 
    $idcarro = $datosCarro['CodCarro'];//encontramos su carro pendiente
    $ins = "delete from carroproductos where CodCarro = ?
			and CodProd = ?";
	$stmt = $bd->prepare($ins);
	$stmt->execute(array($idcarro,$codProd));
	
	if($stmt->rowCount() === 1){	
         $totalNuevo=calcular_total_carrito($_SESSION['carrito']);	
    actualizar_total_carro($totalNuevo);		
		return $stmt->fetch();		
	}else{
		return FALSE;
	}
...
$gmail_del_usuario = $_SESSION['usuario']['gmail'];//obtenemos el gmail del usuario
    ...
    //PASO CLAVE: Obtener el ID 
    $idcarro = $datosCarro['CodCarro'];//encontramos su carro pendiente
    $ins = "update carroproductos set Unidades = ? where CodCarro = ?
			and CodProd = ?";
	$stmt = $bd->prepare($ins);
	$stmt->execute(array($unidades,$idcarro,$codProd));
	if($stmt->rowCount() === 1){
    $totalNuevo=calcular_total_carrito($_SESSION['carrito']);	
    actualizar_total_carro($totalNuevo);	
		return $stmt->fetch();		
...
//funcion que nos actualiza el total en el carro de la bd
...
$bd = leer_config();
	...
	$stmt = $bd->prepare($ins);
	$stmt->execute(array($total,$idcarro));
...
//funcion que actualiza un pedido pendiente cuando añades un producto
