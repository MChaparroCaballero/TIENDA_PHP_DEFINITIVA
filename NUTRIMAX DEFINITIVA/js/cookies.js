// Nombre de la clave para guardar el email en Local Storage
const localStorageClave = "userEmail";

/** Guarda el email del usuario en Local Storage.
 * @param {string} email - El email del usuario.
 */
function guardarEmailEnLocalStorage(email) {
    if (typeof(Storage) !== "undefined") {
        // Guardamos el email. Lo hacemos en minúsculas para consistencia.
        localStorage.setItem(localStorageClave, email.toLowerCase());
    } else {
        console.warn("Local Storage no es soportado en este navegador.");
    }
}

/**Lee el email guardado en Local Storage.
 * @returns {string | null} El email guardado o null si no existe.
 */
function leerEmailDeLocalStorage() {
    if (typeof(Storage) !== "undefined") {
        return localStorage.getItem(localStorageClave);
    }
    return null;
}

/**Rellena el campo de usuario del formulario con el email guardado.*/
function rellenarUsuarioDesdeLocalStorage() {
    const usuarioInput = document.getElementById('usuario');
    const emailGuardado = leerEmailDeLocalStorage();

    // Solo rellenamos si el campo está vacío (para no sobreescribir el valor PHP)
    if (usuarioInput && emailGuardado && usuarioInput.value === "") {
        usuarioInput.value = emailGuardado;
    }
}
...
function iniciarTema() {
    ...
    rellenarUsuarioDesdeLocalStorage();
}
