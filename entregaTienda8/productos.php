...
<html>
	...
	<body class="body">
		...
		// Si el código de categoría no es válido, mostramos un mensaje y un boton de volver a categorias
		if ($codCat === FALSE || $codCat <= 0) {
			...
			require 'footer.php';
            echo "</body></html>";
            exit;//importante que no intente continuar el resto de codigo
        }
		//si la categoria es valida
		try {
			...
			//Mostramos mensaje de error con el error que es
			echo "<p class='alert alert-warning'> " . htmlspecialchars($error_productos) . "</p>";
			echo "</div>";
		...
		//si no hay error construimos la seccion de categorias con bootstrap

		echo "<div id='datosCategoria' class='container my-5'>"; //contenedor principal de esta seccion
		...
		//recorremos todos los productos que tiene la categoria y por cada uno sacamos su datos y hacemos una tarjeta
		foreach($productos as $producto){
			...
			//le agregamos un onclick para que si pinche nos lleve a su detalle con ademas
			//  el codigo del producto que es para mostrar sus datos
